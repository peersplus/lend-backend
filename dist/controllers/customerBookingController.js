...
// ─── Token helpers ────────────────────────────────────────────────────────────
export function generateBookingToken(testDriveId) {
    return createHmac('sha256', env.oauthStateSecret)
        .update(`booking:${testDriveId}`)
        .digest('hex');
}
function verifyBookingToken(testDriveId, token) {
    try {
...
export async function getCustomerBookingController(req, res) {
    ...
    const td = await TestDrive.findOne({ id: testDriveId }).lean();
    if (!td) {
...
export async function cancelCustomerBookingController(req, res) {
    const { testDriveId } = req.params;
    const token = getToken(req);
    if (!token || !verifyBookingToken(testDriveId, token)) {
        res.status(403).json({ data: null, error: { message: 'Invalid or missing booking token.' } });
...
export async function rescheduleCustomerBookingController(req, res) {
    const { testDriveId } = req.params;
    const token = getToken(req);
    if (!token || !verifyBookingToken(testDriveId, token)) {
        res.status(403).json({ data: null, error: { message: 'Invalid or missing booking token.' } });
...
export async function uploadCustomerDocumentController(req, res) {
    const { testDriveId } = req.params;
    const token = getToken(req);
    if (!token || !verifyBookingToken(testDriveId, token)) {
        res.status(403).json({ data: null, error: { message: 'Invalid or missing booking token.' } });
    ...
    const td = await TestDrive.findOne({ id: testDriveId }, { id: 1, customer_id: 1 }).lean();
    if (!td) {
        res.status(404).json({ data: null, error: { message: 'Booking not found.' } });
        return;
    }
    ...
    await Customer.findOneAndUpdate({ id: td.customer_id }, { $set: { driving_license_url: fileUrl, updated_at: new Date().toISOString() } });
    // Non-blocking: notify customer + sales person
    void notifyLicenseUploaded(td.id, td.customer_id, fileUrl).catch((err) => {
...
export async function rebookCustomerController(req, res) {
    const { testDriveId } = req.params;
    const token = getToken(req);
    if (!token || !verifyBookingToken(testDriveId, token)) {
        res.status(403).json({ data: null, error: { message: 'Invalid or missing booking token.' } });
    ...
    const customer = await Customer.findOne({ id: td.customer_id }, { full_name: 1, phone: 1, email: 1 }).lean();
    if (!customer) {
...
export async function notifyLicenseUploaded(testDriveId, customerId, fileUrl) {
    ...
    const scheduledTime = t?.scheduled_time ? t.scheduled_time.substring(0, 5) : '';
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
  ...
  <div style="border:1px solid #e5e7eb;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    ...
    <p style="margin-top:20px;color:#6b7280;font-size:13px;">If you did not upload this document or have any concerns, please contact us immediately.</p>
        ...
        }).catch((err) => console.error('[license] customer email failed:', err?.message));
    }
    // ── 2. In-app + email notification to assigned sales person ──────────────
    if (!t?.assigned_sales_person_id)
        return;
    const salesProfile = await Profile.findOne({ id: t.assigned_sales_person_id }, { user_id: 1, full_name: 1, email: 1 }).lean();
    if (!salesProfile)
        return;
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
  ...
  <div style="border:1px solid #e5e7eb;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    ...
    <p style="color:#6b7280;font-size:11px;margin-top:16px">Test Drive ID: ${testDriveId} · AutoAdvant automated notification</p>
        ...
        }).catch((err) => console.error('[license] sales email failed:', err?.message));
    }
}
