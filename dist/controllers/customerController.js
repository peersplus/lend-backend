...
export async function updateCustomerController(req, res) {
    ...
    const oldLicenseUrl = current.driving_license_url;
    if (newLicenseUrl && newLicenseUrl !== oldLicenseUrl) {
        const activeTd = await TestDrive.findOne({ customer_id: req.params.id, status: { $in: ['scheduled', 'confirmed', 'show', 'rescheduled'] } }, { id: 1 }).sort({ scheduled_date: 1 }).lean();
        if (activeTd?.id) {
            void notifyLicenseUploaded(activeTd.id, req.params.id, newLicenseUrl).catch((err) => {
