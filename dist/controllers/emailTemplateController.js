...
export async function previewEmailTemplateController(req, res) {
    try {
        ...
        if (dealerId) {
            const custom = await EmailTemplateCustomization.findOne({ dealer_id: dealerId, template_key: templateKey }, { subject_override: 1, body_override: 1 }).lean();
            if (custom) {
