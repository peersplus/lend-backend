 ...
 */
export async function submitTestDriveFeedbackController(req, res) {
    try {
        const { test_drive_id, customer_id, enquiry_id, customer_name, customer_email, customer_phone, rating, experience_badge, total_duration_minutes, feedback_text, would_recommend, } = req.body;
        ...
        const vehicleName = vehicle ? `${vehicle.brand} ${vehicle.model}`.trim() : undefined;
        const locationName = location?.name;
        const salesPersonName = salesProfile?.full_name;
        const salesPersonPhone = salesProfile?.phone;
        ...
        if (salesProfile?.email) {
            try {
                await sendTransactionalEmail({
                    recipientEmail: salesProfile.email,
        ...
        if (td?.location_id) {
            ...
            if (adminUserIds.length > 0) {
                const adminProfiles = await Profile.find({ user_id: { $in: adminUserIds }, location_id: td.location_id }, { email: 1, full_name: 1 }).lean();
                for (const admin of adminProfiles) {
                    if (!admin.email || salesProfile?.email === admin.email)
                        continue;
                    try {
                        await sendTransactionalEmail({
