import { deleteConfig, getConfigByLocationId, listConfigs, upsertConfig, } from '../services/followUpReminderConfigService.js';
export async function listFollowUpReminderConfigsController(req, res) {
    try {
...
export async function getFollowUpReminderConfigController(req, res) {
    try {
...
export async function upsertFollowUpReminderConfigController(req, res) {
    try {
        if (!req.authUser?.uid) {
...
export async function deleteFollowUpReminderConfigController(req, res) {
    try {
        if (!req.authUser?.uid) {
            res.status(401).json({ data: null, error: { message: 'Unauthorized' } });
            return;
