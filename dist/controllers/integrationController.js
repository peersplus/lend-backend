...
import { testIntegrationDispatch } from '../services/notificationDispatcherService.js';
const VALID_TYPES = ['whatsapp', 'sms', 'email', 'google_calendar', 'outlook', 'crm', 'dms'];
...
function requireDealerId(req) {
    return req.authUser?.dealer_id ?? null;
}
export async function listAllIntegrationsController(req, res) {
    try {
...
export async function listIntegrationsController(req, res) {
    const dealerId = requireDealerId(req);
    if (!dealerId)
        return res.status(403).json({ data: null, error: { message: 'Dealer context required' } });
    try {
...
export async function upsertIntegrationController(req, res) {
    const dealerId = requireDealerId(req);
    if (!dealerId)
        return res.status(403).json({ data: null, error: { message: 'Dealer context required' } });
    try {
...
export async function deleteIntegrationController(req, res) {
    ...
    if (!VALID_TYPES.includes(type))
        return res.status(400).json({ data: null, error: { message: 'Invalid integration type' } });
    try {
...
export async function testIntegrationController(req, res) {
    const dealerId = requireDealerId(req);
    if (!dealerId)
        return res.status(403).json({ data: null, error: { message: 'Dealer context required' } });
    const type = req.params.type;
    ...
    try {
        ...
        const overrideConfig = (req.body.config ?? {});
