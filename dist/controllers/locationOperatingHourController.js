import { bulkUpsertLocationOperatingHours, createLocationOperatingHour, deleteLocationOperatingHour, getLocationOperatingHourById, listLocationOperatingHours, updateLocationOperatingHour, } from '../services/locationOperatingHourService.js';
...
export async function listLocationOperatingHoursController(req, res) {
    try {
...
export async function getLocationOperatingHourController(req, res) {
    try {
...
export async function createLocationOperatingHourController(req, res) {
    try {
        if (!req.authUser?.uid) {
...
export async function updateLocationOperatingHourController(req, res) {
    try {
        if (!req.authUser?.uid) {
            res.status(401).json({ data: null, error: { message: 'Unauthorized' } });
            return;
...
export async function deleteLocationOperatingHourController(req, res) {
    try {
        if (!req.authUser?.uid) {
            res.status(401).json({ data: null, error: { message: 'Unauthorized' } });
            return;
...
export async function bulkUpsertLocationOperatingHoursController(req, res) {
    try {
        if (!req.authUser?.uid) {
            res.status(401).json({ data: null, error: { message: 'Unauthorized' } });
            return;
        ...
        const data = await bulkUpsertLocationOperatingHours(req.authUser.uid, locationId, rows);
        res.status(200).json({ data, error: null });
    }
    catch (error) {
        const message = error.message;
