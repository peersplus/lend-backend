import { createLocationSpecialPeriod, deleteLocationSpecialPeriod, getLocationSpecialPeriodById, listLocationSpecialPeriods, updateLocationSpecialPeriod, } from '../services/locationSpecialPeriodService.js';
function parseBoolean(value) {
    if (typeof value !== 'string')
        return false;
    return value === 'true' || value === '1';
}
export async function listLocationSpecialPeriodsController(req, res) {
    try {
...
export async function getLocationSpecialPeriodController(req, res) {
    try {
...
export async function createLocationSpecialPeriodController(req, res) {
    try {
        if (!req.authUser?.uid) {
...
export async function updateLocationSpecialPeriodController(req, res) {
    try {
        if (!req.authUser?.uid) {
            res.status(401).json({ data: null, error: { message: 'Unauthorized' } });
            return;
...
export async function deleteLocationSpecialPeriodController(req, res) {
    try {
        if (!req.authUser?.uid) {
            res.status(401).json({ data: null, error: { message: 'Unauthorized' } });
            return;
