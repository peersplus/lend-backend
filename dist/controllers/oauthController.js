...
function verifyState(state) {
    try {
        ...
        if (Date.now() - Number(raw.p.ts) > 10 * 60 * 1000)
            return null;
        return raw.p;
    }
    catch {
...
const OUTLOOK_AUTH_URL = (tenant) => `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/authorize`;
const OUTLOOK_TOKEN_URL = (tenant) => `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token`;
...
export function startGoogleOAuth(req, res) {
    ...
    if (!dealerId)
        return res.status(400).send('dealer_id required');
    if (!env.googleOAuthClientId)
        return res.status(501).send('GOOGLE_OAUTH_CLIENT_ID not configured');
...
export function startOutlookOAuth(req, res) {
    const dealerId = req.query.dealer_id || req.authUser?.dealer_id;
    if (!dealerId)
        return res.status(400).send('dealer_id required');
    if (!env.outlookOAuthClientId)
        return res.status(501).send('OUTLOOK_OAUTH_CLIENT_ID not configured');
    const state = signState({ dealer_id: dealerId, nonce: randomBytes(8).toString('hex'), ts: String(Date.now()) });
    const params = new URLSearchParams({
...
// ─── Callback handlers ────────────────────────────────────────────────────────
const FRONTEND_SETTINGS_URL = (origin) => `${origin}/settings?tab=integrations`;
function closingHtml(success, message) {
...
export async function googleOAuthCallback(req, res) {
    ...
    if (!stateData)
        return res.status(400).send(closingHtml(false, 'Invalid or expired OAuth state'));
    ...
    try {
        ...
        res.send(closingHtml(false, `Failed to connect Google Calendar: ${err.message}`));
    }
}
export async function outlookOAuthCallback(req, res) {
    const { code, state, error } = req.query;
    if (error) {
    ...
    try {
        ...
        res.send(closingHtml(false, `Failed to connect Outlook: ${err.message}`));
    }
}
