import { bulkReassignTestDrives, countTestDrives, createTestDrive, deleteTestDrive, getTestDriveById, listTestDrives, updateTestDrive, } from '../services/testDriveService.js';
export async function getTestDrivesController(req, res) {
    try {
        const filters = {};
        ...
        if (req.query.ids)
            filters.ids = req.query.ids;
        if (req.query.created_at_gte)
            filters.created_at_gte = req.query.created_at_gte;
        if (req.query.date_gte)
            filters.date_gte = req.query.date_gte;
        if (req.query.date_lte)
            filters.date_lte = req.query.date_lte;
        ...
        res.status(500).json({ data: null, count: null, error: { message: error.message } });
    }
}
export async function getTestDriveController(req, res) {
    try {
        const data = await getTestDriveById(req.params.id);
        if (!data) {
            res.status(404).json({ data: null, error: { message: 'Test drive not found' } });
            return;
        }
...
export async function createTestDriveController(req, res) {
    try {
...
export async function updateTestDriveController(req, res) {
    try {
...
export async function deleteTestDriveController(req, res) {
    try {
...
export async function bulkReassignController(req, res) {
    try {
