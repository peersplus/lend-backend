...
export async function getRoleController(req, res) {
    ...
    const data = await userRoleService.getRoleByUserId(userId, req.authUser?.email);
    res.json({ data });
}
export async function listRolesController(req, res) {
    const data = await userRoleService.listUserRoles(req.query);
    res.json({ data });
}
export async function upsertRoleController(req, res) {
    const { user_id, role, notify, userEmail, userName, previousRole } = req.body;
    if (!user_id || !role)
