...
async function run() {
    ...
    const { getAvailableVehiclesForBooking, } = await import('../services/vehicleService.js');
    const { scheduleTransit, dispatchTransit, markTransitArrived, markVehicleReceived, getIncomingTransitsForLocation, } = await import('../services/vehicleFleetService.js');
    // ── Test data constants ────────────────────────────────────────────────────
    ...
    const sharedFound = avail1.shared.find((v) => v.id === vehicleId);
    if (sharedFound) {
        ...
        log(JSON.stringify(avail1.shared.map((v) => v.id)));
    }
    ...
    const sharedAfterConflict = avail2.shared.find((v) => v.id === vehicleId);
    ...
    const sharedAt11 = avail3.shared.find((v) => v.id === vehicleId);
    if (sharedAt11) {
    ...
    const incomingTransit = incoming.find((t) => t.id === transit.id);
    if (incomingTransit) {
    ...
    const sharedAtB = avail4.shared.find((v) => v.id === vehicleId);
