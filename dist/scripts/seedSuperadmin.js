...
async function main() {
    ...
    await UserRole.findOneAndUpdate({ user_id: uid }, { $set: { user_id: uid, role: 'superadmin' }, $setOnInsert: { id: randomUUID() } }, { upsert: true, new: true });
    console.log('UserRole set to superadmin ✓');
    // Upsert Profile (no location_id — superadmin is global)
    const now = new Date().toISOString();
    await Profile.findOneAndUpdate({ user_id: uid }, {
        $set: {
