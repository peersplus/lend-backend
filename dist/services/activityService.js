...
export async function listEvents(filters = {}, limit = 200) {
    const q = {};
    if (filters.user_id)
        q.user_id = filters.user_id;
    if (filters.profile_id)
        q.profile_id = filters.profile_id;
...
export async function touchSession(id, activeSeconds, idleSeconds) {
    await StaffActivitySession.updateOne({ id }, { $set: { last_seen_at: new Date().toISOString(), is_online: true }, $inc: { active_seconds: activeSeconds, idle_seconds: idleSeconds } });
}
export async function endSession(id) {
    await StaffActivitySession.updateOne({ id }, { $set: { logout_at: new Date().toISOString(), is_online: false } });
}
export async function getActiveSessionByUserId(userId) {
...
export async function listOnlineSessions(locationId, locationIds) {
    const q = { is_online: true };
