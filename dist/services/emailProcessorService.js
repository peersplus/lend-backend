import { countFailedAttempts, deleteEmailFromQueue, isAlreadySent, moveEmailToDlq, readEmailBatch, } from './emailQueueService.js';
...
async function processQueue(queue, batchSize, sendDelayMs, ttlMinutes) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        try {
            ...
            if (isRateLimit) {
                ...
                await EmailSendState.findOneAndUpdate({ key: 'global' }, { $set: { retry_after_until: retryUntil, updated_at: new Date() } }, { upsert: true });
...
export async function processEmailQueues() {
    ...
    try {
        ...
        for (const q of ['auth_emails', 'transactional_emails']) {
            ...
            if (result.stopped)
                return { processed: total, stopped: result.stopped };
        }
