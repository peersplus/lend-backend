...
export async function disableFirebaseUser(uid) {
    if (!adminReady())
        throw new Error('Firebase Admin is not initialized.');
    await getAuth().updateUser(uid, { disabled: true });
}
export async function enableFirebaseUser(uid) {
    if (!adminReady())
        throw new Error('Firebase Admin is not initialized.');
 ...
 */
async function getTokensForLocation(locationId, role) {
    let userIds = [];
    if (role) {
        const roles = await UserRole.find({ role }).lean();
        const allUids = roles.map((r) => r.user_id);
...
export async function sendPushToLocation(locationId, payload, role) {
    if (!adminReady())
        return;
...
function buildStatusMessages(status, ctx) {
 ...
 */
export async function notifyTestDriveStatusChange(status, ctx) {
    ...
    const emit = async (userId, payload, profileId) => {
