...
const ALLOWED_ROLES = ['superadmin', 'super_admin', 'dealer_admin', 'sales_admin'];
function lean(doc) {
...
async function resolveActorDealerId(actor) {
    if (actor.dealerId)
...
export async function getConfigByLocationId(locationId) {
...
export async function upsertConfig(userId, data) {
    ...
    if (existing) {
        ...
        const updated = await FollowUpReminderConfig.findOneAndUpdate({ location_id: locationId }, { $set: updates }, { new: true });
...
export async function deleteConfig(userId, locationId) {
