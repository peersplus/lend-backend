...
import { createFirebaseUser, deleteFirebaseUser, generateVerificationLink, getFirebaseUser, getFirebaseUserByEmail, setCustomClaims, } from './firebaseService.js';
...
import { sendDailyTestDriveReports, sendDailyActivityReports, sendTransactionalEmail, logReportSendAttempt, retryFailedReports, } from './reportEmailService.js';
import { EmailUnsubscribeToken } from '../models/EmailUnsubscribeToken.js';
const CREATEABLE_ROLES = ['dealer_admin', 'brand_admin', 'sales_admin', 'branch_admin', 'gro', 'sales', 'security'];
const DEALER_ADMIN_CREATEABLE_ROLES = ['brand_admin', 'sales_admin', 'branch_admin', 'gro', 'sales', 'security'];
const SALES_ADMIN_CREATEABLE_ROLES = ['sales'];
function normalizeRole(input) {
    const raw = String(input ?? '').trim().toLowerCase();
    if (!raw)
        return null;
    const normalized = raw.replace(/[-\s]+/g, '_');
    if (normalized === 'salesadmin')
        return 'sales_admin';
    if (normalized === 'branchadmin')
        return 'branch_admin';
    if (CREATEABLE_ROLES.includes(normalized)) {
        return normalized;
    }
    return null;
}
...
async function resolveDealerIdForUser(userId) {
    const dealerByAdmin = await Dealer.findOne({ admin_user_id: userId }).lean();
    if (dealerByAdmin?.id)
        return String(dealerByAdmin.id);
...
async function createStaffUser(payload, callerUserId) {
    if (!callerUserId) {
    ...
    if (isSalesAdmin && !SALES_ADMIN_CREATEABLE_ROLES.includes(role)) {
    ...
    try {
        await upsertUserRole(created.uid, role);
        await upsertProfile({
            user_id: created.uid,
            full_name: fullName,
            email,
...
async function resolveDealerIdForTargetUser(targetUserId) {
    const directDealer = await Dealer.findOne({ admin_user_id: targetUserId }).lean();
    if (directDealer?.id)
        return String(directDealer.id);
...
async function canAccessTargetUser(callerUserId, targetUserId) {
    const callerRole = await getCallerRole(callerUserId);
    const isSuperAdmin = callerRole === 'superadmin' || callerRole === 'super_admin';
    const isDealerAdmin = callerRole === 'dealer_admin';
    const isSalesAdmin = callerRole === 'sales_admin' || callerRole === 'branch_admin';
...
async function getStaffVerificationStatus(payload, callerUserId) {
    if (!callerUserId)
        throw new Error('Unauthorized');
...
async function resendStaffVerification(payload, callerUserId) {
    if (!callerUserId)
        throw new Error('Unauthorized');
...
async function deleteStaffUser(payload, callerUserId) {
    if (!callerUserId)
        throw new Error('Unauthorized');
    const targetUserId = String(payload.userId ?? '').trim();
    if (!targetUserId)
...
export async function invokeFunction(name, payload, userId) {
    ...
    if (name === 'handle-email-unsubscribe') {
        ...
        if (token) {
            await EmailUnsubscribeToken.findOneAndUpdate({ token }, { $set: { unsubscribed_at: new Date() } });
            return { success: true, method: 'token' };
        }
        else if (email) {
            await EmailUnsubscribeToken.updateMany({ email }, { $set: { unsubscribed_at: new Date() } });
