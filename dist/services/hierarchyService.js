...
// ── Business Units ────────────────────────────────────────────────────────────
export async function listBusinessUnits(filters = {}) {
    const query = {};
    if (filters.orgId)
...
export async function updateBusinessUnit(id, data) {
    const doc = await BusinessUnit.findOneAndUpdate({ id }, { $set: { ...data, updated_at: new Date().toISOString() } }, { new: true });
    if (!doc)
        return null;
    return toPlain(doc);
}
...
// ── Sales Offices ─────────────────────────────────────────────────────────────
export async function listSalesOffices(filters = {}) {
    const query = {};
    if (filters.orgId)
        query.orgId = filters.orgId;
...
export async function getSalesOfficeById(id) {
...
export async function createSalesOffice(data) {
    const now = new Date().toISOString();
...
export async function updateSalesOffice(id, data) {
    const doc = await SalesOffice.findOneAndUpdate({ id }, { $set: { ...data, updated_at: new Date().toISOString() } }, { new: true });
    if (!doc)
        return null;
    return toPlain(doc);
}
export async function deleteSalesOffice(id) {
...
export async function listPlants(filters = {}) {
    ...
    if (filters.businessUnitId)
        query.businessUnitId = filters.businessUnitId;
    if (filters.salesOfficeId)
        query.salesOfficeId = filters.salesOfficeId;
    if (typeof filters.isActive === 'boolean')
        query.isActive = filters.isActive;
...
export async function updatePlant(id, data) {
    const doc = await Plant.findOneAndUpdate({ id }, { $set: { ...data, updated_at: new Date().toISOString() } }, { new: true });
    if (!doc)
        return null;
    return toPlain(doc);
}
