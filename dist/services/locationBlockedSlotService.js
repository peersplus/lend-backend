...
export async function listBlockedSlots(filters = {}) {
    const query = {};
...
export async function getBlockedSlotById(id) {
...
// ── Mutations ─────────────────────────────────────────────────────────────────
export async function createBlockedSlot(userId, data) {
    ...
    if (!startTime || !endTime)
        throw new Error('start_time and end_time are required (HH:MM)');
...
export async function deleteBlockedSlot(userId, id) {
 ...
 */
export async function isTimeRangeBlocked(locationId, date, slotStartTime, slotEndTime) {
 ...
 */
export async function cancelConflictingBookings(userId, blockedSlotId, reason) {
    ...
    await TestDrive.updateMany({ id: { $in: toCancel.map((td) => td.id) } }, {
        $set: {
            status: 'cancelled',
