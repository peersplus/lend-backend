...
async function ensureManagePermission(userId, locationId) {
...
function validateHourRecord(record) {
...
export async function listLocationOperatingHours(filters = {}) {
    const query = {};
...
export async function getLocationOperatingHourById(id) {
...
export async function createLocationOperatingHour(userId, data) {
...
export async function updateLocationOperatingHour(userId, id, data) {
    ...
    const doc = await LocationOperatingHour.findOneAndUpdate({ id }, { $set: { ...data, updated_at: new Date().toISOString() } }, { new: true });
    return doc ? toPlain(doc) : null;
}
export async function deleteLocationOperatingHour(userId, id) {
    const existing = await LocationOperatingHour.findOne({ id }).lean();
    if (!existing)
        return null;
    await ensureManagePermission(userId, existing.location_id);
...
export async function bulkUpsertLocationOperatingHours(userId, locationId, rows) {
    if (!locationId) {
        throw new Error('location_id is required');
    }
    await ensureManagePermission(userId, locationId);
    ...
    for (const row of rows) {
        validateHourRecord(row);
        if (row.id) {
            await LocationOperatingHour.findOneAndUpdate({ id: row.id, location_id: locationId }, {
                $set: {
        ...
        if (existingByDay?.id) {
            await LocationOperatingHour.findOneAndUpdate({ id: existingByDay.id }, {
                $set: {
                    open_time: row.open_time,
                    close_time: row.close_time,
                    is_closed: row.is_closed,
