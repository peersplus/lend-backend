...
export async function listLocationSpecialPeriods(filters = {}) {
    const query = {};
...
export async function getLocationSpecialPeriodById(id, includeDeleted = false) {
    const query = { id };
    if (!includeDeleted) {
        query.is_deleted = { $ne: true };
        query.is_active = { $ne: false };
...
export async function createLocationSpecialPeriod(userId, data) {
...
export async function updateLocationSpecialPeriod(userId, id, data) {
    ...
    const doc = await LocationSpecialPeriod.findOneAndUpdate({ id, is_deleted: { $ne: true } }, { $set: { ...data, updated_at: new Date().toISOString() } }, { new: true });
    if (!doc)
        return null;
    return toPlain(doc);
}
export async function deleteLocationSpecialPeriod(userId, id) {
    const existing = await LocationSpecialPeriod.findOne({ id, is_deleted: { $ne: true } }).lean();
    if (!existing)
        return null;
    await ensureManagePermission(userId, existing.location_id);
    const now = new Date().toISOString();
    const doc = await LocationSpecialPeriod.findOneAndUpdate({ id, is_deleted: { $ne: true } }, { $set: { is_active: false, is_deleted: true, deleted_at: now, updated_at: now } }, { new: true });
