 ...
 */
import { DealerIntegration } from '../models/DealerIntegration.js';
...
// ─── Channel adapters ────────────────────────────────────────────────────────
async function dispatchWhatsApp(cfg, p) {
    ...
    if (provider === 'twilio') {
        const { account_sid, auth_token, from_number } = cfg;
        if (!account_sid || !auth_token || !from_number)
            throw new Error('Twilio WhatsApp: account_sid, auth_token, from_number required');
        ...
        if (!res.ok)
            throw new Error(`Twilio WhatsApp error ${res.status}: ${await res.text()}`);
        return;
    }
    if (provider === 'waba') {
        const { phone_number_id, token } = cfg;
        if (!phone_number_id || !token)
            throw new Error('WABA: phone_number_id and token required');
    ...
    // Custom webhook
    const { api_url, token } = cfg;
    if (!api_url)
        throw new Error('WhatsApp custom: api_url required');
    ...
    if (!res.ok)
        throw new Error(`WhatsApp webhook error ${res.status}`);
}
async function dispatchSMS(cfg, p) {
    const phone = p.customerPhone;
    if (!phone)
        return;
    ...
    if (provider === 'twilio') {
        const { account_sid, auth_token, from_number } = cfg;
        if (!account_sid || !auth_token || !from_number)
            throw new Error('Twilio SMS: account_sid, auth_token, from_number required');
        const creds = Buffer.from(`${account_sid}:${auth_token}`).toString('base64');
        const res = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${account_sid}/Messages.json`, {
            method: 'POST',
            headers: { Authorization: `Basic ${creds}`, 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    // Custom SMS gateway
    const { api_url, api_key, from } = cfg;
    if (!api_url)
        throw new Error('SMS custom: api_url required');
    const res = await fetch(api_url, {
        method: 'POST',
...
async function dispatchEmail(cfg, p) {
    ...
    if (provider === 'sendgrid') {
        const { api_key, from_email, from_name } = cfg;
        if (!api_key)
            throw new Error('SendGrid: api_key required');
...
async function refreshGoogleToken(cfg) {
    ...
    if (!refresh_token || !client_id || !client_secret)
        throw new Error('Google Calendar: refresh_token, client_id, client_secret required for token refresh');
...
async function refreshOutlookToken(cfg) {
    ...
    if (!refresh_token || !client_id || !client_secret)
        throw new Error('Outlook: refresh_token, client_id, client_secret required for token refresh');
...
async function dispatchGoogleCalendar(cfg, p) {
    const c = cfg;
    if (p.event === 'test_drive_cancelled')
        return; // deletion needs stored event IDs
    if (!c.refresh_token && !c.access_token)
        throw new Error('Google Calendar: access_token or refresh_token required');
    ...
    let res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendar_id)}/events`, { method: 'POST', headers: { Authorization: `Bearer ${access_token}`, 'Content-Type': 'application/json' }, body });
    // Token expired mid-session — refresh and retry once
    if (res.status === 401 && c.refresh_token) {
        access_token = await refreshGoogleToken(c);
        res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendar_id)}/events`, { method: 'POST', headers: { Authorization: `Bearer ${access_token}`, 'Content-Type': 'application/json' }, body });
    }
    if (!res.ok)
        throw new Error(`Google Calendar error ${res.status}: ${await res.text()}`);
}
async function dispatchOutlook(cfg, p) {
    const c = cfg;
    if (p.event === 'test_drive_cancelled')
        return;
    if (!c.refresh_token && !c.access_token)
        throw new Error('Outlook: access_token or refresh_token required');
    // Always prefer a fresh token when refresh_token is available
    let access_token = c.refresh_token
    ...
    if (!res.ok)
        throw new Error(`Outlook error ${res.status}: ${await res.text()}`);
}
async function dispatchWebhook(label, cfg, p) {
    const { webhook_url, api_key, secret_header, secret_value } = cfg;
    if (!webhook_url)
        throw new Error(`${label}: webhook_url required`);
    ...
    if (!res.ok)
        throw new Error(`${label} webhook error ${res.status}`);
}
// ─── Main dispatcher ─────────────────────────────────────────────────────────
export async function dispatchNotification(payload) {
    try {
        ...
        if (!dealerId) {
            const loc = await Location.findOne({ id: payload.locationId }, { dealer_id: 1 }).lean();
            dealerId = loc?.dealer_id ?? undefined;
        }
        if (!dealerId)
            return;
        const integrations = await DealerIntegration.find({ dealer_id: dealerId, is_enabled: true }).lean();
        if (!integrations.length)
            return;
        await Promise.allSettled(integrations
            .filter(i => !i.events?.length || i.events.includes(payload.event))
            .map(async (i) => {
            try {
                const cfg = (i.config ?? {});
                switch (i.type) {
                    case 'whatsapp':
                        await dispatchWhatsApp(cfg, payload);
                        break;
                    case 'sms':
                        await dispatchSMS(cfg, payload);
                        break;
                    case 'email':
                        await dispatchEmail(cfg, payload);
                        break;
                    case 'google_calendar':
                        await dispatchGoogleCalendar(cfg, payload);
                        break;
                    case 'outlook':
                        await dispatchOutlook(cfg, payload);
                        break;
                    case 'crm':
                        await dispatchWebhook('CRM', cfg, payload);
                        break;
                    case 'dms':
                        await dispatchWebhook('DMS', cfg, payload);
                        break;
                }
            }
            catch (err) {
                console.error(`[integration:${i.type}] dispatch failed:`, err.message);
            }
        }));
    }
    catch (err) {
 ...
 */
export async function testIntegrationDispatch(type, cfg, samplePayload) {
    try {
        switch (type) {
            case 'whatsapp':
                await dispatchWhatsApp(cfg, samplePayload);
                break;
            case 'sms':
                await dispatchSMS(cfg, samplePayload);
                break;
            case 'email':
                await dispatchEmail(cfg, samplePayload);
                break;
            case 'google_calendar':
                await dispatchGoogleCalendar(cfg, samplePayload);
                break;
            case 'outlook':
                await dispatchOutlook(cfg, samplePayload);
                break;
            case 'crm':
                await dispatchWebhook('CRM', cfg, samplePayload);
                break;
            case 'dms':
                await dispatchWebhook('DMS', cfg, samplePayload);
                break;
        }
        return { success: true };
    }
