...
export async function markAsRead(id, userId) {
    await Notification.updateOne({ id, user_id: userId }, { $set: { is_read: true, read_at: new Date().toISOString() } });
}
export async function markAllAsRead(userId) {
    await Notification.updateMany({ user_id: userId, is_read: false }, { $set: { is_read: true, read_at: new Date().toISOString() } });
}
