...
export async function listProfiles(filters = {}) {
    ...
    if (filters.role) {
        ...
        const userIds = matchingRoles.map((r) => r.user_id).filter(Boolean);
        if (userIds.length === 0)
            return [];
...
export async function upsertProfile(data) {
    ...
    const doc = await Profile.findOneAndUpdate({ user_id: data.user_id }, { $set: { ...data, id, updated_at: now }, $setOnInsert: { created_at: now } }, { upsert: true, new: true });
    return lean(doc);
}
export async function updateProfile(id, data) {
    const doc = await Profile.findOneAndUpdate({ id }, { $set: { ...data, updated_at: new Date().toISOString() } }, { new: true });
    return doc ? lean(doc) : null;
}
export async function setProfileActive(userId, is_active) {
 ...
 */
export async function setLeave(id, opts) {
 ...
 */
export async function clearExpiredLeaves() {
    const today = new Date().toISOString().split('T')[0];
...
export async function touchLastLogin(userId) {
    await Profile.updateOne({ user_id: userId }, { $set: { last_login_at: new Date().toISOString(), updated_at: new Date().toISOString() } });
}
