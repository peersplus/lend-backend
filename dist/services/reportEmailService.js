...
function generateReportHTML(report) {
    ...
    const salesRows = report.salesPeople.map(p => `<tr><td style="padding:10px;font-size:13px;">${p.name}</td>
     ...
     <td style="padding:10px;text-align:center;font-size:13px;color:#ef4444;">${p.no_show}</td></tr>`).join('');
    const secRows = report.security.map(p => `<tr><td style="padding:10px;font-size:13px;">${p.name}</td>
     <td style="padding:10px;text-align:center;font-size:13px;">${p.checked_in}</td>
     <td style="padding:10px;text-align:center;font-size:13px;">${p.checked_out}</td></tr>`).join('');
    const groRows = report.gro.map(p => `<tr><td style="padding:10px;font-size:13px;">${p.name}</td>
     <td style="padding:10px;text-align:center;font-size:13px;">${p.assigned}</td>
     <td style="padding:10px;text-align:center;font-size:13px;">${p.completed}</td></tr>`).join('');
    const activityRows = Object.entries(report.activitySummary.eventTypes).map(([type, count]) => `<tr><td style="padding:8px;font-size:13px;">${type}</td>
     <td style="padding:8px;text-align:center;font-size:13px;font-weight:600;">${count}</td></tr>`).join('');
...
// ── Report Data Builder ───────────────────────────────────────────────────────
async function buildReportData(locationId, reportDate) {
    ...
    for (const e of events) {
        ...
        if (e.role) {
            if (!roleActivity[e.role])
                roleActivity[e.role] = { events: 0, sessions: 0 };
...
export async function sendDailyTestDriveReports(options = {}) {
    ...
    for (const loc of locations) {
        ...
        try {
            await DailyTestDriveReport.findOneAndUpdate({ location_id: loc.id, report_date: reportDate, report_type: 'test_drive_daily' }, {
            ...
            if (!data) {
                await DailyTestDriveReport.updateOne({ location_id: loc.id, report_date: reportDate, report_type: 'test_drive_daily' }, { $set: { status: 'failed', error_message: 'Location data not found' } });
                continue;
            }
            ...
            await DailyTestDriveReport.updateOne({ location_id: loc.id, report_date: reportDate, report_type: 'test_drive_daily' }, { $set: { status: 'sent', sent_at: new Date() } });
            queued++;
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            errors.push(`${loc.id}: ${msg}`);
            await DailyTestDriveReport.updateOne({ location_id: loc.id, report_date: reportDate, report_type: 'test_drive_daily' }, { $set: { status: 'failed', error_message: msg.slice(0, 500) } });
        }
    }
...
export async function logReportSendAttempt(args) {
    ...
    if (args.errorMessage)
        update.error_message = args.errorMessage.slice(0, 500);
    if (args.status === 'sent')
        ...
        try {
            ...
            if (srcBranding?.dealerName || srcBranding?.dealerLogoUrl) {
                ...
                try {
                    ...
                    const locationId = (args.templateData?.location_id || args.templateData?.locationId);
                    const dealerId = args.templateData?._dealerId;
                    let dealerDoc = null;
                    if (locationId) {
                        const loc = await Location.findOne({ id: locationId }, { dealer_id: 1 }).lean();
                        if (loc?.dealer_id) {
                            dealerDoc = await Dealer.findOne({ id: loc.dealer_id }, { name: 1, logo_url: 1 }).lean();
                        }
                    }
                    else if (dealerId) {
                    ...
                    if (dealerDoc) {
                        brandingFields = {
                            _dealerName: dealerDoc.name || undefined,
                            _dealerLogoUrl: dealerDoc.logo_url || undefined,
                        };
                    }
                }
            ...
            try {
                const resolvedDealerId = brandingFields._dealerId
                ...
                const locationId = (args.templateData?.location_id || args.templateData?.locationId);
                let dealerIdForCustom = null;
                if (locationId) {
                    const loc = await Location.findOne({ id: locationId }, { dealer_id: 1 }).lean();
                    dealerIdForCustom = loc?.dealer_id || null;
                }
                if (dealerIdForCustom) {
                    const custom = await EmailTemplateCustomization.findOne({ dealer_id: dealerIdForCustom, template_key: args.templateName }, { subject_override: 1, body_override: 1 }).lean();
                    if (custom) {
                        if (custom.subject_override)
                            subject = custom.subject_override;
                        if (custom.body_override)
                            html = custom.body_override;
                    }
                }
            }
            ...
            if (!html)
                throw new Error(`Template '${args.templateName}' not found and no html provided`);
        }
    }
    if (!subject || !html) {
