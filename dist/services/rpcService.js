...
function requiredString(args, keys, fieldName) {
...
export async function runRpc(name, args) {
    ...
    const contactPhone = readArg(args, '_contact_phone', 'contact_phone');
    const businessUnitRaw = readArg(args, '_business_unit', 'business_unit') || {};
    ...
    const brandList = (readArg(args, '_brands', 'brands') || [])
        .map((brand) => {
    ...
    const rawLocations = readArg(args, '_locations', 'locations') || [];
    ...
    const locationList = rawLocations.map((location, index) => {
        ...
        if (!locationCode)
            throw new Error(`locations[${index}].locationCode is required`);
        if (!salesOfficeCode)
            throw new Error(`locations[${index}].salesOfficeCode is required`);
        if (!plantCode)
            throw new Error(`locations[${index}].plantCode is required`);
        return {
            name: locationName,
            locationCode,
            externalLocationId: typeof location.externalLocationId === 'string' && location.externalLocationId.trim()
            ...
            salesOfficeName: typeof location.salesOfficeName === 'string' && location.salesOfficeName.trim()
                ? location.salesOfficeName.trim()
                : null,
            salesOfficeCode,
            externalSalesOfficeId: typeof location.externalSalesOfficeId === 'string' && location.externalSalesOfficeId.trim()
                ? location.externalSalesOfficeId.trim()
                : null,
            plantName: typeof location.plantName === 'string' && location.plantName.trim()
                ? location.plantName.trim()
                : null,
            plantCode,
            externalPlantId: typeof location.externalPlantId === 'string' && location.externalPlantId.trim()
    ...
    const createdBusinessUnitIds = [];
    const createdSalesOfficeIds = [];
    const createdPlantIds = [];
    const createdBusinessUnitBrandIds = [];
    ...
    try {
        ...
        const salesOfficeByCode = new Map();
        const plantByCompositeKey = new Map();
        ...
        if (existingProfile) {
            await upsertProfile(existingProfile);
        }
        else {
