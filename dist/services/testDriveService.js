...
import { dispatchNotification } from './notificationDispatcherService.js';
...
async function resolveDealerBranding(locationId) {
    if (!locationId)
        return {};
    try {
        ...
        if (!loc?.dealer_id)
            return {};
        const dealer = await Dealer.findOne({ id: loc.dealer_id }, { name: 1, logo_url: 1 }).lean();
        if (!dealer)
            return {};
        return {
...
export async function listTestDrives(filters = {}) {
    const query = {};
    ...
    if (filters.sales_person_id)
        query.assigned_sales_person_id = filters.sales_person_id;
    if (filters.assigned_sales_person_id)
        query.assigned_sales_person_id = filters.assigned_sales_person_id;
    if (filters.assigned_gro_id)
        query.assigned_gro_id = filters.assigned_gro_id;
    if (filters.status)
        query.status = filters.status;
    if (filters.scheduled_date)
        query.scheduled_date = filters.scheduled_date;
    ...
    if (filters.created_at_gte)
        query.created_at = { $gte: String(filters.created_at_gte) };
    if (filters.date_gte || filters.date_lte) {
        const dateQ = {};
        if (filters.date_gte)
            dateQ.$gte = String(filters.date_gte);
        if (filters.date_lte)
            dateQ.$lte = String(filters.date_lte);
    ...
    const assignedProfileIds = Array.from(new Set(rows
        .flatMap((row) => [row.assigned_sales_person_id, row.assigned_gro_id, row.assigned_security_person_id])
        .filter(Boolean)));
...
async function checkPublicBookingRateLimit(customerId, locationId) {
    const location = await Location.findOne({ id: locationId }, { public_booking_rate_limit_minutes: 1 }).lean();
    const rateLimitMinutes = typeof location?.public_booking_rate_limit_minutes === 'number' &&
    ...
    if (recentCount > 0) {
        throw new Error(`You can only submit one booking request every ${rateLimitMinutes} minute${rateLimitMinutes === 1 ? '' : 's'}. Please try again later.`);
    }
}
...
export async function createPublicTestDrive(data) {
    ...
    if (customer) {
        ...
        const patch = {};
        if (data.full_name && data.full_name !== customer.full_name)
            patch.full_name = data.full_name;
        if (data.email !== undefined && data.email !== (customer.email ?? null))
            patch.email = data.email ?? null;
        const preferredContact = data.preferred_contact ?? 'phone';
        if (preferredContact !== customer.preferred_contact)
            patch.preferred_contact = preferredContact;
...
async function checkVehicleSlotAvailability(vehicleId, scheduledDate, scheduledTime, slotDurationMinutes) {
    const vehicle = await Vehicle.findOne({ id: vehicleId }, { available_units: 1 }).lean();
...
export async function createTestDrive(data) {
    ...
    const assignedGroId = data.assigned_gro_id ?? data.gro_id ?? null;
    const cancelledReason = data.cancelled_reason ?? data.cancellation_reason ?? null;
    // ── Check vehicle slot availability to prevent double-booking ────────────
    if (data.vehicle_id && data.scheduled_date && data.scheduled_time) {
        const slotDuration = typeof data.slot_duration_minutes === 'number' && data.slot_duration_minutes > 0
            ? data.slot_duration_minutes
            : 30;
        const { available, availableUnits, bookedCount } = await checkVehicleSlotAvailability(String(data.vehicle_id), String(data.scheduled_date), String(data.scheduled_time), slotDuration);
        if (!available) {
            throw new Error(`This vehicle is fully booked for the selected time slot (${bookedCount}/${availableUnits} units taken). Please choose a different slot or vehicle.`);
        }
    }
    // ── Auto-assign sales person if not explicitly set ────────────────────────
    let resolvedSalesPersonId = data.assigned_sales_person_id ?? null;
    let autoAssignedProfile = null;
    ...
    const doc = new TestDrive({
        ...
        slot_duration_minutes: typeof data.slot_duration_minutes === 'number' && data.slot_duration_minutes > 0
            ? data.slot_duration_minutes
            : 30,
...
export async function updateTestDrive(id, data) {
    const assignedGroId = data.assigned_gro_id ?? data.gro_id;
    const cancelledReason = data.cancelled_reason ?? data.cancellation_reason;
    const patch = {
        ...data,
    ...
    const doc = await TestDrive.findOneAndUpdate({ id }, { $set: patch }, { new: true });
    if (!doc)
        return null;
    const plain = toPlain(doc);
    // Fire FCM push + Firestore real-time event whenever status is being set
    if (typeof data.status === 'string') {
        void afterStatusChange(plain, data.status).catch((err) => {
        ...
        if (completionStatuses.includes(data.status)) {
            ...
            if (vehicle?.is_shared) {
                void autoTransitAfterDrive(plain.vehicle_id, plain.location_id, plain.id, new Date()).catch(() => null);
            }
        }
    }
    return plain;
...
export async function bulkReassignTestDrives(fromProfileId, toProfileId, date) {
...
export async function countTestDrives(filters = {}) {
    const query = {};
    if (filters.location_id)
        query.location_id = filters.location_id;
    if (filters.location_ids && Array.isArray(filters.location_ids) && filters.location_ids.length > 0) {
...
async function notifyStaffStatusChange(opts) {
    const { td, customerName, vehicleName, locationName, dateLabel, salesPersonEmail, salesPersonName, headline, detail, badge, dealerName } = opts;
    const staffHtml = (recipientName, role) => `
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
  ...
  <div style="border:1px solid #e5e7eb;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    ...
    if (td.location_id) {
        ...
        const locationProfiles = await Profile.find({ location_id: td.location_id }, { id: 1, user_id: 1, full_name: 1, email: 1 }).lean();
        if (locationProfiles.length) {
            const userIds = locationProfiles.map((p) => p.user_id).filter(Boolean);
            const roleRows = await UserRole.find({ user_id: { $in: userIds }, role: { $in: [...adminRoles] } }, { user_id: 1, role: 1 }).lean();
            ...
            for (const p of locationProfiles) {
                ...
                if (emailsToNotify.some((e) => e.to === p.email))
                    continue;
...
async function afterStatusChange(td, status) {
    ...
    const salesPersonName = salesProfile?.full_name || '';
    const salesPersonEmail = salesProfile?.email;
    // ── Email: cancelled ──────────────────────────────────────────────────────
    if (status === 'cancelled') {
        ...
        const cancelDealerName = cancelBranding._dealerName;
        // Customer email
        if (customerEmail) {
    ...
    if (status === 'rescheduled') {
        ...
        const rescheduleDealerName = rescheduleBranding._dealerName;
        // Customer email
        if (customerEmail) {
    ...
    await notifyTestDriveStatusChange(status, {
        ...
        customerName,
        vehicleName,
        assignedSalesUserId: salesProfile?.user_id,
        assignedGroUserId: groProfile?.user_id,
...
async function writeTestDriveEvent(data) {
    if (!getApps().length || !data.location_id)
        return;
    try {
...
async function sendTestDriveBookedNotifications(td) {
    ...
    if (c?.email) {
        ...
        await sendMail({
            ...
            html: tmpl.html,
            _dealerName: branding._dealerName,
        }).catch((err) => {
    ...
    const locationProfiles = td.location_id
    ...
    const roleRows = userIds.length
        ...
        : [];
    const roleMap = new Map(roleRows.map((r) => [r.user_id, r.role]));
    const profileByUserId = new Map(locationProfiles.map((p) => [p.user_id, p]));
    const adminRoles = new Set(['dealer_admin', 'sales_admin', 'branch_admin', 'superadmin', 'super_admin']);
    const notifyRoles = new Set(['gro', 'security']);
    const staffBranding = await resolveDealerBranding(td.location_id);
    const staffDealerName = staffBranding._dealerName;
    ...
    for (const [uid, role] of roleMap) {
        ...
        if (p.id === td.assigned_sales_person_id)
            continue;
        await sendMail({
            to: p.email,
...
export async function autoAssignSalesPerson(locationId) {
    const today = new Date().toISOString().split('T')[0];
    ...
    }, { id: 1, user_id: 1, full_name: 1, email: 1 }).lean();
    if (!locationProfiles.length)
        return null;
    // Keep only those with 'sales' role
    const userIds = locationProfiles.map((p) => p.user_id).filter(Boolean);
    const salesRoles = await UserRole.find({ user_id: { $in: userIds }, role: 'sales' }, { user_id: 1 }).lean();
    const salesUserIds = new Set(salesRoles.map((r) => r.user_id));
    const salesProfiles = locationProfiles.filter((p) => salesUserIds.has(p.user_id));
    ...
    const salesProfileIds = salesProfiles.map((p) => p.id);
    const activeCounts = await TestDrive.aggregate([
    ...
    const sorted = [...salesProfiles].sort((a, b) => (loadMap.get(a.id) ?? 0) - (loadMap.get(b.id) ?? 0));
...
async function sendSalesAssignmentEmail(td, salesProfile) {
    ...
    await sendMail({
        ...
        _dealerName: assignBranding._dealerName,
    }).catch(() => null);
}
// ─── Email templates ─────────────────────────────────────────────────────────
function testDriveCustomerEmailHtml(p) {
...
function testDriveStaffEmailHtml(p) {
    return `
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a">
