 ...
 */
export async function routeBetweenLocations(fromId, toId) {
    if (fromId === toId)
 ...
 */
export async function getVehicleAvailabilityAtLocation(vehicleId, toLocationId, date) {
    ...
    const effectiveLocationId = String(vehicle.current_location_id || vehicle.location_id);
 ...
 */
export async function scheduleTransit(opts) {
    ...
    await Vehicle.findOneAndUpdate({ id: opts.vehicleId }, {
        $set: {
 ...
 */
export async function dispatchTransit(transitId) {
    const now = new Date().toISOString();
    const doc = await VehicleTransit.findOneAndUpdate({ id: transitId, status: { $in: ['scheduled'] } }, { $set: { status: 'in_transit', dispatched_at: now, updated_at: now } }, { new: true });
    if (!doc)
        return null;
 ...
 */
export async function markTransitArrived(transitId) {
    const now = new Date().toISOString();
    const transit = await VehicleTransit.findOneAndUpdate({ id: transitId, status: { $in: ['scheduled', 'in_transit'] } }, { $set: { status: 'arrived', arrived_at: now, updated_at: now } }, { new: true });
    if (!transit)
        return null;
    // Update vehicle: move to new location, clear transit state
    await Vehicle.findOneAndUpdate({ id: transit.vehicle_id }, {
        $set: {
 ...
 */
export async function cancelTransit(transitId, reason) {
    const now = new Date().toISOString();
    const doc = await VehicleTransit.findOneAndUpdate({ id: transitId, status: { $in: ['scheduled'] } }, { $set: { status: 'cancelled', notes: reason ?? null, updated_at: now } }, { new: true });
    if (!doc)
        return null;
    ...
    if (otherActive === 0) {
        await Vehicle.findOneAndUpdate({ id: doc.vehicle_id }, { $set: { transit_status: 'at_location', transit_eta: null, transit_to_location_id: null, updated_at: now } });
    }
    return lean(doc);
}
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export async function autoTransitAfterDrive(vehicleId, fromLocationId, completedTestDriveId, endedAt) {
    ...
    if (!nextDrive || nextDrive.location_id === fromLocationId) {
        ...
        await Vehicle.findOneAndUpdate({ id: vehicleId }, {
            $set: {
...
// Fleet overview
// ─────────────────────────────────────────────────────────────────────────────
export async function getFleetOverview(dealerId, locationId) {
    // Get all shared vehicles
    const vehicleQuery = { is_shared: true, is_active: true };
    if (dealerId) {
    ...
    vehicles.forEach((v) => {
        ...
        if (v.current_location_id)
            allLocIds.add(v.current_location_id);
    });
    ...
    const locMap = {};
    locationDocs.forEach((l) => { locMap[l.id] = l; });
    // Resolve receiver names for active transits
    const receiverIds = [...new Set(activeTransits.map((t) => t.receiver_profile_id).filter(Boolean))];
    ...
    receiverProfiles.forEach((p) => { receiverNameMap[String(p.id)] = p.full_name; });
    const transitMap = {};
    activeTransits.forEach((t) => {
        if (!transitMap[String(t.vehicle_id)])
            transitMap[String(t.vehicle_id)] = [];
    ...
    const driveMap = {};
    upcomingDrives.forEach((d) => {
        if (!driveMap[String(d.vehicle_id)])
            driveMap[String(d.vehicle_id)] = [];
...
export async function listTransits(filters) {
    ...
    if (filters.vehicle_id)
        q.vehicle_id = filters.vehicle_id;
    if (filters.to_location_id)
        q.to_location_id = filters.to_location_id;
    if (filters.receiver_profile_id)
        q.receiver_profile_id = filters.receiver_profile_id;
    if (filters.status) {
    ...
    if (filters.from_date || filters.to_date) {
        ...
        if (filters.from_date)
            range.$gte = filters.from_date;
        if (filters.to_date)
            range.$lte = filters.to_date;
        q.depart_time = range;
    }
 ...
 */
export async function getSecurityAtLocation(locationId) {
    // Find user_ids with 'security' role
    const securityRoles = await UserRole.find({ role: 'security' }, { user_id: 1 }).lean();
    const userIds = securityRoles.map((r) => String(r.user_id));
    if (userIds.length === 0)
        return [];
    ...
    return profiles.map((p) => { const o = { ...p }; delete o._id; return o; });
}
/**
 ...
 */
async function sendReceiverEmail(transitId, receiverProfileId) {
    try {
        ...
        if (!transit || !receiverProfile?.email)
            return;
        ...
        const fmtDt = (iso) => new Date(iso).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true });
 ...
 */
export async function autoAssignReceiver(transitId) {
    ...
    const today = new Date().toISOString().split('T')[0];
    const counts = await Promise.all(securityList.map(async (s) => {
    ...
    const now = new Date().toISOString();
    await VehicleTransit.updateOne({ id: transitId }, { $set: { receiver_profile_id: assignedProfileId, receiver_assigned_at: now, updated_at: now } });
 ...
 */
export async function assignReceiver(transitId, profileId) {
    const now = new Date().toISOString();
    const doc = await VehicleTransit.findOneAndUpdate({ id: transitId, status: { $in: ['scheduled', 'in_transit'] } }, { $set: { receiver_profile_id: profileId, receiver_assigned_at: now, updated_at: now } }, { new: true });
    if (!doc)
        return null;
 ...
 */
export async function markVehicleReceived(transitId, receiverProfileId, notes) {
    const transit = await VehicleTransit.findOne({ id: transitId }).lean();
    if (!transit)
    ...
    const isAuthorized = String(transit.receiver_profile_id) === receiverProfileId ||
        securityAtDest.some((s) => s.id === receiverProfileId);
    if (!isAuthorized) {
    ...
    await VehicleTransit.updateOne({ id: transitId }, {
        $set: {
 ...
 */
export async function getIncomingTransitsForLocation(locationId) {
    ...
    const vehicleIds = [...new Set(transits.map((t) => String(t.vehicle_id)))];
    const fromLocIds = [...new Set(transits.map((t) => String(t.from_location_id)))];
    const receiverIds = transits
        .map((t) => t.receiver_profile_id)
        .filter(Boolean);
    ...
    const lMap = {};
    fromLocs.forEach((l) => { lMap[l.id] = l; });
    const rMap = {};
    receivers.forEach((r) => { rMap[r.id] = r; });
...
async function getManagersAtLocation(locationId) {
    const managerRoles = await UserRole.find({ role: { $in: ['dealer_admin', 'sales_admin'] } }, { user_id: 1, role: 1 }).lean();
    if (managerRoles.length === 0)
        return [];
    const userIds = managerRoles.map((r) => String(r.user_id));
    const profiles = await Profile.find({ user_id: { $in: userIds }, location_id: locationId, is_active: true }, { id: 1, full_name: 1, email: 1, phone: 1, user_id: 1, location_id: 1 }).lean();
    return profiles.map((p) => { const o = { ...p }; delete o._id; return o; });
}
 ...
 */
export async function createTransitRequest(opts) {
    const now = new Date().toISOString();
 ...
 */
export async function approveTransitRequest(requestId, managerProfileId, managerNotes) {
    ...
    await VehicleTransitRequest.updateOne({ id: requestId }, {
        $set: {
            status: 'approved',
 ...
 */
export async function rejectTransitRequest(requestId, managerProfileId, managerNotes) {
    const request = await VehicleTransitRequest.findOne({ id: requestId, status: 'pending' }).lean();
    if (!request)
        throw new Error('Request not found or already actioned');
...
export async function cancelTransitRequest(requestId, requesterProfileId) {
    ...
    const nowIso = new Date().toISOString();
    await VehicleTransitRequest.updateOne({ id: requestId }, { $set: { status: 'cancelled', updated_at: nowIso } });
    const updated = await VehicleTransitRequest.findOne({ id: requestId }).lean();
    return updated ? lean(updated) : null;
}
/**
...
export async function listTransitRequests(filters) {
    ...
    if (filters.from_location_id)
        q.from_location_id = filters.from_location_id;
    if (filters.to_location_id)
        q.to_location_id = filters.to_location_id;
    if (filters.requested_by_profile_id)
        q.requested_by_profile_id = filters.requested_by_profile_id;
    if (filters.dealer_id)
        q.dealer_id = filters.dealer_id;
    if (filters.status) {
        q.status = Array.isArray(filters.status) ? { $in: filters.status } : filters.status;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
async function sendTransitRequestEmails(requestId, event) {
    try {
        ...
        const vehicleLabel = vehicle
            ? `${vehicle.brand} ${vehicle.model}${vehicle.variant ? ` ${vehicle.variant}` : ''}${vehicle.registration_number ? ` (${vehicle.registration_number})` : ''}`
            : 'a vehicle';
        const fromLabel = fromLoc ? `${fromLoc.name}${fromLoc.city ? `, ${fromLoc.city}` : ''}` : 'source branch';
        const toLabel = toLoc ? `${toLoc.name}${toLoc.city ? `, ${toLoc.city}` : ''}` : 'destination branch';
        const baseTable = `
      <table style="width:100%;border-collapse:collapse;font-size:14px;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb;">
        ...
        <tr><td style="padding:9px 14px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;">Requested By</td><td style="padding:9px 14px;border-bottom:1px solid #e5e7eb;">${requester?.full_name || '—'}</td></tr>
        ...
        const wrap = (title, subtitle, accentColor, body) => `
      ...
      <div style="max-width:560px;margin:32px auto;background:#fff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
        ...
        if (event === 'created') {
            ...
            for (const mgr of sourceMgrs) {
                if (!mgr.email)
                    continue;
                const html = wrap('🚗 Transit Request — Approval Needed', `${requester?.full_name || 'A staff member'} at ${toLabel} is requesting a vehicle from your branch`, 'linear-gradient(135deg,#b45309,#f59e0b)', `<p style="margin:0 0 16px">Hi <strong>${mgr.full_name}</strong>,</p>
           ...
           <div style="margin-top:16px;padding:12px 16px;background:#fef3c7;border:1px solid #fde68a;border-radius:8px;font-size:13px;color:#92400e;">
            ...
            for (const sec of sourceSec) {
                if (!sec.email)
                    continue;
                const html = wrap('📋 FYI: Transit Request for Your Location', `A vehicle at ${fromLabel} may be dispatched pending manager approval`, 'linear-gradient(135deg,#1e3a8a,#3b82f6)', `<p style="margin:0 0 16px">Hi <strong>${sec.full_name}</strong>,</p>
           ...
           <p style="margin:16px 0 0;font-size:13px;color:#6b7280;">No action required from you at this time. You will be notified if the request is approved.</p>`);
            ...
            if (requester?.email) {
                const html = wrap('✅ Transit Request Submitted', `Your request for ${vehicleLabel} has been sent to ${fromLabel} for approval`, 'linear-gradient(135deg,#065f46,#10b981)', `<p style="margin:0 0 16px">Hi <strong>${requester.full_name}</strong>,</p>
        ...
        if (event === 'approved') {
            const actioner = request.actioned_by_profile_id
                ...
                : null;
            const managerName = actioner?.full_name || 'The branch manager';
            // To requester: "Approved! Vehicle en route"
            if (requester?.email) {
                const html = wrap('🎉 Transit Approved — Vehicle En Route!', `${managerName} at ${fromLabel} has approved your request`, 'linear-gradient(135deg,#065f46,#10b981)', `<p style="margin:0 0 16px">Hi <strong>${requester.full_name}</strong>,</p>
           ...
           <p style="margin:16px 0 0;font-size:13px;color:#6b7280;">Security at your location will be notified to receive the vehicle.</p>`);
            ...
            for (const sec of destSec) {
                if (!sec.email)
                    continue;
                const html = wrap('🚛 Incoming Vehicle — Prepare to Receive', `Approved transit from ${fromLabel} heading to ${toLabel}`, 'linear-gradient(135deg,#1e40af,#6366f1)', `<p style="margin:0 0 16px">Hi <strong>${sec.full_name}</strong>,</p>
           ...
           <div style="margin-top:16px;padding:12px 16px;background:#fef3c7;border:1px solid #fde68a;border-radius:8px;font-size:13px;color:#92400e;">
            ...
            for (const sec of sourceSec) {
                if (!sec.email)
                    continue;
                const html = wrap('🚗 Vehicle Dispatch Approved', `${vehicleLabel} is being dispatched from ${fromLabel}`, 'linear-gradient(135deg,#374151,#6b7280)', `<p style="margin:0 0 16px">Hi <strong>${sec.full_name}</strong>,</p>
        ...
        if (event === 'rejected') {
            ...
            const managerName = actioner?.full_name || 'The branch manager';
            if (requester?.email) {
                const html = wrap('❌ Transit Request Rejected', `${managerName} at ${fromLabel} was unable to approve this request`, 'linear-gradient(135deg,#991b1b,#ef4444)', `<p style="margin:0 0 16px">Hi <strong>${requester.full_name}</strong>,</p>
