...
import { getVehicleAvailabilityAtLocation } from './vehicleFleetService.js';
function lean(doc) {
...
export async function listVehicles(filters = {}) {
    ...
    if (typeof filters.is_available === 'boolean')
        q.is_available = filters.is_available;
    if (filters.brand)
        q.brand = filters.brand;
    if (filters.model)
...
export async function updateVehicle(id, data) {
    const payload = normalizeVehiclePayload(data);
    const doc = await Vehicle.findOneAndUpdate({ id }, { $set: { ...payload, updated_at: new Date().toISOString() } }, { new: true });
    return doc ? lean(doc) : null;
}
export async function deleteVehicle(id) {
    await Vehicle.deleteOne({ id });
}
export async function bulkInsertVehicles(records) {
    const now = new Date().toISOString();
 ...
 */
export async function getAvailableVehiclesForBooking(locationId, date, // YYYY-MM-DD — optional; if omitted, no slot conflict checks run
time) {
    ...
    const localWithAvailability = await Promise.all(localVehicles.map(async (v) => {
    ...
    const sharedWithAvailability = await Promise.all(sharedVehicles.map(async (v) => {
        const slotInfo = await getVehicleAvailabilityAtLocation(v.id, locationId, effectiveDate);
        const conflicting = date && requestedMinutes !== null
            ? await hasSlotConflict(v.id, effectiveDate, requestedMinutes, 30)
            : false;
...
async function hasSlotConflict(vehicleId, date, startMinutes, durationMinutes = 30) {
    ...
    for (const d of drives) {
        ...
        // Overlap check
        if (startMinutes < existingEnd && newEnd > existing)
            return true;
    }
    return false;
}
