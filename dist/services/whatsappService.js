...
function buildUserIdFromPhone(phone) {
...
function parseUrgency(input) {
    const value = String(input || '').trim().toLowerCase();
    if (['2', 'urgent', 'high', 'now', 'asap'].includes(value))
        return 'urgent';
    if (['1', 'normal', 'regular'].includes(value))
        return 'normal';
    return null;
}
function parsePriceMode(input) {
    const value = String(input || '').trim().toLowerCase();
    if (['1', 'free'].includes(value))
        return 'free';
    if (['2', 'rent', 'rental'].includes(value))
        return 'rent';
    return null;
}
...
async function setSession(phone, flow, step, data) {
    ...
    await WhatsAppSession.findOneAndUpdate({ id: existing.id }, {
        $set: {
            flow,
            step,
            data,
...
async function sendMenuAndReset(phone) {
    await setSession(phone, 'menu', 'awaiting_menu', {});
    await sendWhatsAppText(phone, MENU_TEXT);
}
async function handleLendFlow(phone, step, data, text) {
    ...
    if (step === 'lend_location') {
        ...
        const finalData = { ...data, address: value };
        try {
...
async function handleRequestFlow(phone, step, data, text) {
    const value = normalizeText(text);
    ...
    if (step === 'request_urgency') {
        ...
        try {
            ...
            if (ranked.length) {
                ...
                const lines = ranked.map((item, idx) => `${idx + 1}. ${item?.title || 'Item'} (${item?.category || 'Other'})`);
...
async function handleOtherFlow(phone, text) {
    const value = normalizeText(text);
    if (!value) {
