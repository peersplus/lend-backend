...
export function wrapEmailHtml(subject, bodyHtml) {
    const safeSubject = String(subject || 'PeersPlus');
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    ...
    .header { padding: 18px 32px; background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%); border-bottom: 1px solid #e4e4e7; }
    .body { padding: 32px; }
    ...
    .footer-logo-wrap { display: block; margin-top: 12px; text-align: center; }
    .footer-logo { display: inline-block; width: 80px !important; max-width: 80px !important; height: auto; }
...
<body>
  ...
  <div class="container">
    <div class="header">
      <p style="margin:0;font-size:12px;color:#71717a;">PeersPlus updates</p>
    </div>
    <div class="body">
      ${bodyHtml}
    </div>
    <div class="footer">
      ...
      <a class="footer-logo-wrap" href="${PEERSPLUS_SITE_URL}" target="_blank" rel="noreferrer">
        <img class="footer-logo" src="${PEERSPLUS_LOGO_URL}" alt="PeersPlus" width="80" style="display:inline-block;width:80px;max-width:80px;height:auto;" />
      </a>
    </div>
  </div>
</body>
...
function base(previewText, bodyContent, branding = {}) {
    ...
    const headerContent = logoUrl
        ? `<img src="${logoUrl}" alt="${brand}" style="height:24px;max-width:90px;object-fit:contain;display:block;" />`
...
function extractBranding(data) {
    return {
        dealerName: data._dealerName,
        dealerLogoUrl: data._dealerLogoUrl,
        primaryColor: data._primaryColor,
    };
}
// ── Templates ────────────────────────────────────────────────────────────────
export function testDriveCompletedTemplate(data) {
    const { customerName, vehicleName, locationName, scheduledDate, salesPersonName, durationMinutes } = data;
    const branding = extractBranding(data);
    return {
        subject: `Your test drive is complete — thank you, ${customerName || 'there'}!`,
        html: base(`Your test drive of ${vehicleName || 'a vehicle'} is complete — thank you!`, `<div class="body">
        ...
        <div class="details">
          ...
          ${detailRow('Assisted by', salesPersonName)}
...
export function testDriveRescheduledTemplate(data) {
    const { customerName, vehicleName, locationName, newDate, newTime } = data;
    const branding = extractBranding(data);
    return {
        subject: `Test drive rescheduled — ${newDate || 'new date confirmed'}`,
        html: base(`Your test drive has been rescheduled`, `<div class="body">
...
export function testDriveCancelledTemplate(data) {
    const { customerName, vehicleName, reason, bookingUrl, manageUrl } = data;
    const branding = extractBranding(data);
    return {
        subject: `Test drive cancellation confirmed`,
        html: base('Your test drive has been cancelled — rebook at your convenience', `<div class="body">
...
export function bookingConfirmationTemplate(data) {
    const { customerName, vehicleName, locationName, scheduledDate, scheduledTime, salesPersonName, manageBookingUrl } = data;
    const branding = extractBranding(data);
    return {
...
export function salesFollowUpTemplate(data) {
    const { customerName, salesPersonName, vehicleName, followUpNote } = data;
    const branding = extractBranding(data);
    const brandName = branding.dealerName || SITE_NAME;
    return {
        subject: `Follow-up from ${brandName}`,
        html: base(`Your ${brandName} sales consultant is following up`, `<div class="body">
...
export function salesAssignmentTemplate(data) {
    const { salesPersonName, customerName, vehicleName, scheduledDate, scheduledTime } = data;
    const branding = extractBranding(data);
    return {
        subject: `New test drive assigned — ${customerName}`,
        html: base(`You have a new test drive assignment`, `<div class="body">
...
export function testDriveJourneyTemplate(data) {
    const { customerName, vehicleName, locationName, locationAddress, locationPhone, scheduledDate, scheduledTime, salesPersonName, currentStatus, feedbackLink, totalDurationMinutes, 
    // legacy fields kept for backwards-compat
    durationMinutes: legacyDuration, startKm, endKm, notes, } = data;
    const branding = extractBranding(data);
    ...
    return {
        ...
        html: base(`Journey summary for your test drive`, `<div class="body">
        ...
        <div class="details">
          ${detailRow('Vehicle', vehicleName)}
          ${detailRow('Location', locationName)}
          ${locationAddress ? detailRow('Address', locationAddress) : ''}
          ${detailRow('Date', scheduledDate)}
          ${detailRow('Time', scheduledTime)}
...
export function staffWelcomeTemplate(data) {
    const { fullName, roleLabel, verificationLink, loginUrl } = data;
    const branding = extractBranding(data);
    const brandName = branding.dealerName || SITE_NAME;
    return {
        subject: `Verify your account to sign in`,
        html: base(`Welcome to ${brandName} — verify your account`, `<div class="body">
...
export function vehicleChangeNotificationTemplate(data) {
    const { customerName, oldVehicle, newVehicle, scheduledDate } = data;
    const branding = extractBranding(data);
    return {
        subject: `Vehicle updated for your test drive`,
        html: base('Your test drive vehicle has been updated', `<div class="body">
...
export function demoRequestConfirmationTemplate(data) {
    const { contactName, dealerName } = data;
    const branding = extractBranding(data);
    return {
        subject: `Demo request received — ${SITE_NAME}`,
        html: base('We received your demo request', `<div class="body">
...
export function testDriveReminder24hTemplate(data) {
    const { customerName, vehicleName, locationName, scheduledDate, scheduledTime, salesPersonName } = data;
    const branding = extractBranding(data);
    return {
        subject: `⏰ Your test drive is tomorrow — ${vehicleName || 'get ready!'}`,
        html: base(`Your test drive is confirmed for tomorrow!`, `<div class="body">
...
export function testDriveReminder4hTemplate(data) {
    const { customerName, vehicleName, locationName, scheduledDate, scheduledTime, salesPersonName } = data;
    const branding = extractBranding(data);
    return {
        subject: `🚀 See you in ~4 hours — ${vehicleName || 'test drive'} today!`,
        html: base(`Your test drive starts in about 4 hours`, `<div class="body">
...
export function testDriveNoShowReengagementTemplate(data) {
    const { customerName, vehicleName, locationName, bookingUrl, manageUrl } = data;
    const branding = extractBranding(data);
    return {
        subject: `We missed you today, ${customerName || 'there'} — rebook your test drive 🚗`,
        html: base(`We missed you! Rebook your test drive at your convenience.`, `<div class="body">
...
export function testDriveThankYouTemplate(data) {
    const { customerName, vehicleName, locationName, salesPersonName, salesPersonPhone, bookingUrl } = data;
    const branding = extractBranding(data);
    return {
        subject: `Thank you for your test drive, ${customerName || 'there'}! 🎉`,
        html: base(`Thank you for visiting us — here's what's next.`, `<div class="body">
...
// ── Registry ─────────────────────────────────────────────────────────────────
export function testDriveFeedbackReceivedTemplate(data) {
    const { customerName, vehicleName, locationName, rating, experienceBadge, feedbackText, wouldRecommend, submittedAt } = data;
    const branding = extractBranding(data);
    ...
    return {
        ...
        html: base(`Customer feedback received`, `<div class="body">
...
export function testDriveFeedbackThankYouTemplate(data) {
    const { customerName, vehicleName, locationName, rating, salesPersonName, salesPersonPhone } = data;
    const branding = extractBranding(data);
    const stars = '⭐'.repeat(Number(rating) || 0);
    return {
        subject: `Thank you for your feedback, ${customerName || 'there'}! ${stars}`,
        html: base(`Thank you for sharing your test drive experience`, `<div class="body">
        <p>Hi ${customerName || 'there'},</p>
